...
async function main() {
    const createIssueInAdvance = moment_1.default.duration(core.getInput("create_issue_in_advance", { required: true }));
    ...
    for (const shift of pagerdutyShifts) {
        ...
        if (shift.end < moment_1.default()) {
            console.log("Not making an issue. Shift is already over.");
            continue;
        }
        if (shift.start > moment_1.default().add(createIssueInAdvance)) {
            console.log(`Not making an issue. Shift starts in more than ${createIssueInAdvance.toISOString()}.`);
            continue;
        }
        const matchingExistingIssues = existingIssues.filter(issue => { var _a; return (_a = issue.body) === null || _a === void 0 ? void 0 : _a.includes(shift.annotation()); });
